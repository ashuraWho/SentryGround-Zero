...
def test_ingest_product_success(ingestion_manager, setup_teardown_ingest):
    ...
    # Create mathematically valid Numpy data so the Physics validator doesn't crash
    import numpy as np
    dummy_data = np.zeros((100, 100, 3), dtype=np.float32)
    np.save(data_file, dummy_data)
        
    metadata = {
        "product_id": product_id,
        "timestamp": "2023-01-01T00:00:00",
        "sensor": "Sentinel-2",
        "signature_hex": "dummy_signature_123",
        "anomaly": False
    }
...
def test_ingest_invalid_schema(ingestion_manager, setup_teardown_ingest):
    ...
    import numpy as np
    np.save(data_file, np.zeros((10, 10, 3), dtype=np.float32))
