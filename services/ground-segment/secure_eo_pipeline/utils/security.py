...
def generate_key() -> None:
    """
    Generates a new Hybrid PQC + AES-256 symmetric encryption key.
    
    RATIONALE:
    To protect against "Harvest Now, Decrypt Later" quantum attacks, this system 
    simulates a NIST-compliant Hybrid Key Encapsulation Mechanism (KEM) using 
    FIPS 203 (ML-KEM / Kyber). It wraps a classic Fernet/AES key inside a PQC format.
    """
    # Generate the standard AES-based Fernet Key
    classic_key = Fernet.generate_key()
    
    # ---------------------------------------------------------
    # PQC HYBRID ENCAPSULATION SIMULATION
    # ---------------------------------------------------------
    # In a real post-quantum rollout (e.g., using liboqs), we would combine 
    # the classic key with a Kyber-768 encapsulated shared secret.
    # Here, we prepend a simulated 32-byte PQC encapsulation header.
    pqc_header = b"PQC_KYBER768_HYBRID_KEM_HEADER__"
    hybrid_key = pqc_header + classic_key
    
    with open(config.KEY_PATH, "wb") as key_file:
        key_file.write(hybrid_key)
    ...
    print(f"[SECURITY CORE] SUCCESS: A new Hybrid PQC encryption key was written to: {config.KEY_PATH}")


def load_key() -> bytes:
    """
    Retrieves the existing encryption key from the filesystem and extracts the classic key.
    """
    ...
    try:
        with open(config.KEY_PATH, "rb") as key_file:
            hybrid_key = key_file.read()
            
        # Verify and strip the PQC simulated header
        if hybrid_key.startswith(b"PQC_KYBER768_HYBRID_KEM_HEADER__"):
            classic_key = hybrid_key[32:]
            return classic_key
        else:
            # Legacy key fallback
            return hybrid_key
...
def rotate_keys(archive_dir: str, backup_dir: str) -> bool:
    ...
    # 2. Generate new key
    # Emulate the new key generation logic bypassing load_key fallback wrapper
    classic_new = Fernet.generate_key()
    pqc_header = b"PQC_KYBER768_HYBRID_KEM_HEADER__"
    new_hybrid_key_bytes = pqc_header + classic_new
    new_fernet = Fernet(classic_new)
    print("[CRYPTO] New Hybrid PQC key generated in memory.")
    ...
    try:
        with open(config.KEY_PATH, "wb") as f:
            f.write(new_hybrid_key_bytes)
        print("[CRYPTO] SUCCESS: New Hybrid PQC key committed to keystore.")
        return True
    except Exception as e:
        print(f"[CRYPTO] CRITICAL: Re-encryption done but failed to save new key: {e}")
        print(f"[CRYPTO] EMERGENCY DUMP OF NEW KEY: {new_hybrid_key_bytes.decode(errors='replace')}")
        return False
